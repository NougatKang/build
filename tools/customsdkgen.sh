...
if [ -z "$OUT" ]; then
    echo -e "Please lunch a product before using this command"
    exit 1
else
    OUTDIR=${OUT%/*/*/*}
fi
...
echo -e "android.jar created at ${OUTDIR}/android.jar"
echo -e "Now attempting to create new sdk platform with it"

if [ -z "$ANDROID_HOME" ]; then
    ANDROID=$(command -v emulator)
    ANDROID_HOME=${ANDROID%/*}
    if [ -z "$ANDROID_HOME" ]; then
        echo -e "ANDROID_HOME variable is not set. Do you have the sdk installed ?"
        exit 1
    fi
fi
...
if [ -f ${ANDROID_HOME}/platforms/android-${SDK_VER}-${CUSTOM_NAME}/android.jar ]; then
    echo -e "New SDK platform with custom android.jar created inside ${ANDROID_HOME}"
fi
